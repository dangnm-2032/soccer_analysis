...
def evaluate_results(consolidated_dict, gt_dict, full_results = None):
    ...
    illegible_mistake_count = 0 # FN
    illegible_gt_count = 0 # FP
    for m in mistakes:
    ...
    # Precision, Recall, F1
    precision = correct / (correct + illegible_gt_count) if (correct + illegible_gt_count) > 0 else 0.0
    recall = correct / (correct + illegible_mistake_count) if (correct + illegible_mistake_count) > 0 else 0.0
    f1_score = (
